...
export default function AdminDashboard() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      else if (['partnerships','companies','contacts','outreach','templates','followups','documents','partnership_settings','wave1','inbox','delivery_events'].includes(section)) {
        const partnership = await api('partnerships');
        const key = section === 'outreach' ? 'emails' : section === 'partnership_settings' ? 'settings' : section === 'wave1' ? 'campaign_members' : section;
...
function CrmAction({section,crm,reload}:{section:string,crm:any,reload:()=>void}) {
  ...
  const companies=crm?.companies||[]; const contacts=crm?.contacts||[]; const documents=crm?.documents||[];
  ...
  async function submit(event:React.FormEvent<HTMLFormElement>){
    ...
    if(section==='outreach'){path='partnerships/emails';body={...raw,attachment_ids:form.getAll('attachment_ids').filter(Boolean),send_now:true,follow_up_at:raw.follow_up_at||null};}
    if(section==='partnership_settings'){path='partnerships/settings';method='PATCH';}
    try{if(section==='documents'){const file=form.get('document') as File|null;if(!file||file.size===0)throw new Error('Select a document to upload.');const authorization=await api('partnerships/documents/upload-token',{method:'POST'});if(file.size>authorization.max_bytes)throw new Error('Document exceeds the 10 MB limit.');const response=await fetch(authorization.upload_url,{method:'POST',headers:{'X-Wiftco-Upload-Token':authorization.upload_token},body:form});const result=await response.json().catch(()=>({}));if(!response.ok)throw new Error(result.detail||'Document upload failed.');}else {if(section==='outreach'&&!window.confirm('Send this email to the selected prospect now?')){setBusy(false);return;}await api(path,{method,headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});}setNotice(section==='outreach'?'Email and selected attachments were accepted for delivery.':'Saved successfully.');setOpen(false);reload();}
...
function CrmFields({section,companies,contacts,documents,settings}:{section:string,companies:AnyRow[],contacts:AnyRow[],documents:AnyRow[],settings:any}) {
  ...
  if(section==='outreach') {
    ...
    const availableContacts=contacts.filter(c=>!company||c.company===company.name);
    const availableDocuments=documents.filter(d=>d.company==='General'||!company||d.company===company.name);
    return <div className="form-grid"><label>Company<select name="company_id" required value={selectedCompany} onChange={event=>setSelectedCompany(event.target.value)}><option value="" disabled>Select company</option>{companies.map(c=><option key={c.id} value={c.id}>{c.name}</option>)}</select></label><label>Contact<select name="contact_id" required defaultValue="" key={selectedCompany} disabled={!selectedCompany}><option value="" disabled>{selectedCompany?'Select contact':'Select a company first'}</option>{availableContacts.map(c=><option key={c.id} value={c.id}>{c.name} — {c.email}</option>)}</select></label><label className="span-2">Subject<input name="subject" required placeholder="Wiftco × {{company_name}}"/></label><label className="span-2">Message<textarea name="body" required rows={8} placeholder="Hello {{first_name}}, ..."/></label><fieldset className="span-2 attachment-picker"><legend>Attachments <small>(optional)</small></legend>{!selectedCompany?<p>Select a company to see authorized documents.</p>:availableDocuments.length===0?<p>No authorized documents are available. Upload one in Documents first.</p>:availableDocuments.map(d=><label key={d.id}><input name="attachment_ids" type="checkbox" value={d.id}/><span><b>{d.title}</b><small>{d.company} · {d.type}</small></span></label>)}</fieldset><label>CC<input name="cc" placeholder="Optional, comma-separated"/></label><label>BCC<input name="bcc" placeholder="Optional, comma-separated"/></label><label>Follow-up date<input name="follow_up_at" type="datetime-local"/></label></div>;
  }
...
function CampaignReview({rows,crm,reload}:{rows:AnyRow[],crm:any,reload:()=>void}) {
  const [selected,setSelected]=useState<AnyRow|null>(null); const [message,setMessage]=useState(''); const [busy,setBusy]=useState(false);
  async function action(name:string, extra:AnyRow={}){if(!selected)return;setBusy(true);setMessage('');try{await api(`partnerships/campaign-members/${selected.id}/action`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:name,...extra})});setMessage(name==='send'?'Partnership email accepted by Resend.':'Action completed.');if(name==='attach_deck')setSelected({...selected,deck_ready:true,attachments:['Wiftco Telecom Partnership Technical Capability Deck']});if(name==='save_draft')setSelected({...selected,workflow:'admin_review'});if(name==='send_test')setSelected({...selected,workflow:'test_sent'});if(name==='approve')setSelected({...selected,workflow:'ready_to_send'});reload();if(name==='send')setSelected(null);}catch(e:any){setMessage(e.message);}finally{setBusy(false)}}
  return <><div className="campaign-warning"><b>Human approval required</b><span>No Wave 1 email is sent automatically. The partnership deck, test email and explicit recipient confirmation are mandatory.</span></div><article className="panel full"><PanelTitle title={`Wave 1 outreach (${rows.length})`}/><DataTable rows={rows} onToggle={setSelected} actionLabel="Open draft"/></article>{selected&&<div className="modal-backdrop" onMouseDown={()=>setSelected(null)}><section className="crm-modal campaign-review" onMouseDown={e=>e.stopPropagation()}><header><div><span className="eyebrow">ADMIN REVIEW</span><h2>{selected.company}</h2></div><button onClick={()=>setSelected(null)}>×</button></header><div className="review-meta"><span><b>Recipient</b>{selected.recipient}</span><span><b>From</b>{selected.from_email}</span><span><b>Reply-to</b>partnerships@wiftco.com</span><span><b>Subject</b>{selected.subject}</span><span><b>Attachments</b>{selected.attachments?.join(', ')||'Partnership deck required before sending'}</span><span><b>Workflow</b>{pretty(selected.workflow)}</span></div><iframe title="Rendered partnership email" sandbox="" srcDoc={selected.rendered_email}/>{!selected.deck_ready&&<label className="deck-select">Campaign deck<select defaultValue="" onChange={e=>e.target.value&&action('attach_deck',{document_id:e.target.value})}><option value="" disabled>Select uploaded deck</option>{(crm?.documents||[]).map((d:AnyRow)=><option key={d.id} value={d.id}>{d.title}</option>)}</select></label>}<div className="review-actions"><button className="tiny-button" disabled={busy} onClick={()=>action('save_draft')}>Save Draft</button><button className="tiny-button" disabled={busy||!selected.deck_ready} onClick={()=>action('send_test',{test_recipient:selected.owner})}>Send Test</button><button className="tiny-button" disabled={busy||selected.workflow!=='test_sent'} onClick={()=>action('approve')}>Approve Draft</button><button className="primary-button" disabled={busy||selected.workflow!=='ready_to_send'} onClick={()=>{if(window.confirm(`Send this partnership email to ${selected.recipient}? This cannot be undone.`))action('send',{confirmation_recipient:selected.recipient})}}>Send Partnership Email</button></div>{message&&<div className="form-error">{message}</div>}</section></div>}</>;
}
